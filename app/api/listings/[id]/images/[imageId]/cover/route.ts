// app/api/listings/[id]/images/[imageId]/cover/route.ts
...
import { setListingImageCover } from "@/src/server/modules/listings/listingImages.service";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string; imageId: string }> }) {
  try {
    ...
    if (!Number.isFinite(imageIdNum) || imageIdNum <= 0) throw new AppError("imageId inválido", 400);
    ...
    const out = await setListingImageCover({
      listingId,
      siteId: site.id,
      imageId: imageIdNum,
    });
